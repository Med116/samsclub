...
/**
 * @author markdavis
 * This class is responsible for grouping held seats together
 * by generating random numbers to associate to held seats
 * The random # is stored as a member variable so that upon request
 * the user of the service can retreive it, in order
 * to reserve the seats they held
 *
 */
public class SeatHold {
	...
	/**
	 *  holds seats, generates a hold id and sets it to this object,
	 *  and the individual seats held, so we can look up seats by
	 *  the hold id for the TicketServiceImpl::reserveSeats call
	 */
	public void holdSeats() {
		int holdId = getRandomId();
		Util.seatIndex.stream()
			.limit(getSeatCount()) // limits stream to the number of seats of users request
			.forEach(seat -> {
				seat.hold();
	...
	private int getRandomId() {
		// TODO: get distinct list of seathold ids already issued, to make sure its not already used
	...
	/**
	 * Sets the expiration of the hold to the future moment defined in `Util.HOLD_SECONDS` 
	 */
